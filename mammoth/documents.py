...
def paragraph(
    ...
    return Paragraph(
        children, style_id, style_name, numbering, alignment=alignment, indent=indent
    )
...
def hyperlink(children, href=None, anchor=None, target_frame=None):
    return Hyperlink(
        href=href, anchor=anchor, target_frame=target_frame, children=children
    )
...
def table_cell_unmerged(children, colspan, rowspan, vmerge):
    return TableCellUnmerged(
        children=children, colspan=colspan, rowspan=rowspan, vmerge=vmerge
    )
...
def notes(notes_list):
    return Notes(dict((_note_key(note), note) for note in notes_list))
